...
- Snapshot status: `MEASURED / REVIEW_PASSED_WITH_CONDITIONS`
- Workspace: `D:\研究\tiny-object-research`
- Current research branch: `codex/research-evidence-audit`
- Reviewed experiment commit: `95dbcd6176b173183aa38fd0972e5d1075b9434e`
...
B1 相对 B0 的两 seed A1 证据已完成正确复算：平均 `Delta APvt_official_1500=+0.0103`、`Delta ARvt_2_8_3000=+0.0131`、`Delta AP=+0.0297`，两个 seed 方向一致。核心 Gate 已通过；仅剩结果报告数字/hash 同步和测试表述收缩这一机械条件。

## 6. 当前证据边界

- 官方 `APvt` 两 seed 改善：`MEASURED / CONDITIONALLY_ACCEPTED`，平均差 `+0.0103`；
- 精确 2–8 px `ARvt` 两 seed 改善：`MEASURED / CONDITIONALLY_ACCEPTED`，平均差 `+0.0131`；
- 2–4、4–6、6–8 px 漏检下降：`NOT_TESTED`；
- P2 在独立确认 seed 上稳定：`MEASURED / CONDITIONALLY_ACCEPTED`；
...
- PRT-001-A1：`REVIEW_PASSED_WITH_CONDITIONS`；核心 Gate 已通过。只需 report-only commit 同步 ARvt 数字、full hash 和坐标测试表述，无需重训、复评或补 seed 2。
...
- 状态：`REVIEW_PASSED_WITH_CONDITIONS / REPORT_CORRECTION_ONLY`
- 已返回：正确 evaluator、B0/B1 seed 0/1 metrics、summary、Gate 和测试，实验 commit `95dbcd6`；
- 当前许可：仅修正结果报告中的 ARvt 数字、full hash 和坐标测试表述；禁止修改 metrics/Gate，禁止重训、复评或补 seed 2；
...
- 当前结果审查：`research/reviews/2026-08-26-PRT-001-A1-result-review-2.md`、`research/reviews/2026-08-23-PRT-002-result-review-2.md`

若旧的 `2026-08-23-PRT-001-result-review-final.md`、PRT-001-A1 v1.0、design-review-1、result-review-1 或实验侧自评与本快照冲突，以 DR-004、任务卡 v1.1、design-review-2 和 `2026-08-26-PRT-001-A1-result-review-2.md` 为准；旧记录保留用于审计，不删除。
